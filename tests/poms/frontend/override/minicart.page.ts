...
class MiniCartPage {
    ...
    async goToCheckout() {
        await this.toCheckoutButton.evaluate((element: Element) => {
            (element as HTMLElement).click();
        });
        await expect(this.page).toHaveURL(slugs.checkout.checkoutSlug);
    }

    async goToCart() {
        await this.toCartButton.evaluate((element: Element) => {
            (element as HTMLElement).click();
        });
        await expect(this.page).toHaveURL(slugs.cart.cartSlug);
    }

    async removeProductFromMinicart(product: string) {
        let removeProductMiniCartButton = this.page.locator(`button[aria-label*="${product}"]`);

        await removeProductMiniCartButton.evaluate((element: Element) => {
